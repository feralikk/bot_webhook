...
from wood_informant.models import Statuses, Products, Cities, Regions, Countries, Types, WoodTypes, Wets, Actual, \
    Previous, SubTypes, WoodSubTypes
...
def log_errors(f):
    def inner(*args, **kwargs):
        try:
            ...
            file_content1 = ('<{}> <{}>  <{}> <{}> <{}>  <{}>\n'.format(datetime.now(), f.__name__, type(e).__name__, kwargs, Exception, e))
...
class WoodParser(object):
    def __init__(self):
        ...
        self.session.headers = {'User-Agent': choice(desktop_agents),'Accept':'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8'}
    ...
    def parse_urls(self, product_status):
        ...
        page_count = count_soup.find("div", class_="pagination").findAll("a")[-5].text.strip()
        # for p in range(1, 3):
        ...
        for p in range(1, int(page_count) + 1):
            ...
            for item in a_link:
                if item.get("href").find("show") != -1:
                    url_list.append("https://woodresource.ru" + item.get("href"))  # В url_list храним список ссылок
        return url_list
    @log_errors
    def parse_post(self):
        locale.setlocale(locale.LC_TIME, "rus_rus")  # the ru locale is installed
        start_time = time.time()
        product_statuses = ["PagonageSell", "PagonageBuy", "PelletsSell", "PelletsBuy", "LumberSell", "LumberBuy",]
        ...
        for pr_status in product_statuses:
            ...
            for url in url_list:
                ...
                city_tmp = None
                post_id=price=0
                status=product=title=region=country=discription=post_url=""
                subtype=type_lm=wood_type=wet=length=width=thickness=post_date=post_views=city=wood_subtype = None
                additional_regions = False
                title_old=city_old=region_old=country_old=price_old=type_old=wood_type_old=wet_old=length_old = None
                width_old=thickness_old=discription_old=post_views_old=post_url_old=subtype_old=wood_subtype_old = None
                ...
                all_attr = soup.find('div', class_="", id="content-after-sidebar").findAll('p')
                small_attr = soup.find('div', class_="", id="content-after-sidebar").findAll('small')
                for item in small_attr:
                    publish_date += (item.get_text())
                datetime_publish = (publish_date[:publish_date.find("Просмотров:")])
                datetime_publish = (
                    datetime_publish.replace("января", "январь")
                ...
                datetime_publish_obj = datetime.strptime('2000-01-01 00:00:00', '%Y-%m-%d %H:%M:%S')
                try:
                    datetime_publish_obj = datetime.strptime(datetime_publish, "%d %B %Y г. %H:%M")
                except Exception as e:
                    print(e)
                start_date = (datetime.strptime('2022-02-24 00:00:00', '%Y-%m-%d %H:%M:%S'))
                post_date = datetime_publish_obj
                ...
                filtered_text = filtered_text.replace("РоссияЗакрыть", "").replace("УкраинаЗакрыть", "")

                parse_id = soup.find('div', class_="breadcrumb active").find("span").text.strip()
                ...
                title = (soup.find("div", class_="", id="content-after-sidebar").find("h1").text.strip())
                ...
                if city_text.find("(") != -1 and city_text.count(',') == 2:
                    ...
                    region_tmp = city_text[city_text.find(',')+1:city_text.rfind(',')].strip()
                ...
                post_cost_str = filtered_text[filtered_text.find("Цена"):filtered_text.find("Описание:")]
                try:
                    price = (float(post_cost_str[post_cost_str.find(":") + 1:post_cost_str.find("₽")].strip()))
                except Exception as e_rub:
                    price = 0.0
                    logging.error(e_rub)
                if product_tmp == "Пиломатериалы":
                    type_str_tmp = filtered_text[filtered_text.find("Тип:") + 4:filtered_text.find("Порода древесины:")].strip()
                    type_tmp = type_str_tmp[:type_str_tmp.find(":")].strip()
                    subtype_tmp = type_str_tmp[type_str_tmp.find(":") + 1:].strip()
                    try:
                        type_lm, _ = Types.objects.get_or_create(
                            type=type_tmp
                        )
                    except:
                        type_lm = None
                    try:
                        subtype, _ = SubTypes.objects.get_or_create(
                            subtype=subtype_tmp
                        )
                    except:
                        subtype = None
                if product_tmp == "Погонаж":
                    type_tmp = filtered_text[filtered_text.find("Вид:") + 4:filtered_text.find("Порода древесины:")].strip()
                    try:
                        type_lm, _ = Types.objects.get_or_create(
                            type=type_tmp
                        )
                ...
                if product_tmp == "Пеллеты":
                    type_tmp = filtered_text[filtered_text.find("Тип:") + 4:filtered_text.find("Цена")].strip()
                    try:
                        type_lm, _ = Types.objects.get_or_create(
                            type=type_tmp
                        )
                        ...
                        type_lm = None
                if product_tmp != "Пеллеты":
                    wood_type_str_tmp = filtered_text[filtered_text.find("Порода древесины:") + 17:filtered_text.find("Влажность:")].strip()
                    wood_type_tmp = wood_type_str_tmp[:wood_type_str_tmp.find(":")].strip()
                    wood_subtype_tmp = wood_type_str_tmp[wood_type_str_tmp.find(":")+1:].strip()
                    try:
                    ...
                    try:
                        ...
                        wood_subtype = None
                if product_tmp == "Пиломатериалы":
                    wet_tmp = filtered_text[filtered_text.find("Влажность:") + 10:filtered_text.find("Длина в мм:")].strip()
                    try:
                        wet, _ = Wets.objects.get_or_create(
                            wet=wet_tmp
                        )
                    except:
                        wet = None
                if product_tmp == "Погонаж":
                    wet_tmp = filtered_text[filtered_text.find("Влажность:") + 10:filtered_text.find("Цена")].strip()
                    try:
                        wet, _ = Wets.objects.get_or_create(
                            wet=wet_tmp
                        )
                        ...
                        wet = None
                if product_tmp == "Пиломатериалы":
                    length = filtered_text[filtered_text.find("Длина в мм:") + 11:filtered_text.find("Ширина в мм:")].strip()
                    width = filtered_text[filtered_text.find("Ширина в мм:") + 12:filtered_text.find("Толщина в мм:")].strip()
                    if filtered_text[filtered_text.find("Толщина в мм:"):].find("Наличие:") != -1:
                        thickness = filtered_text[filtered_text.find("Толщина в мм:") + 13:filtered_text.find("Наличие:")].strip()
                        post_have = filtered_text[filtered_text.find("Наличие:") + 8:filtered_text.find("Цена")].strip()
                    else:
                        post_have = (" ")
                        thickness = filtered_text[filtered_text.find("Толщина в мм:") + 13:filtered_text.find("Цена")].strip()
                all_discription = all_discription.replace("<br>", " ").replace("</br>", " ").replace("<br/>", " ")
